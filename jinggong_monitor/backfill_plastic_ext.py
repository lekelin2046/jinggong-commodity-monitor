...
------------------------------------------------------------------
  · 价格优先取 `middlePrice`（主流价），与主流程 `pick_price()` 同哲学。
    ⚠️ **middlePrice 不等于 (lowPrice+highPrice)/2** —— 它是隆众单独维护的
       主流价。2026-09-18 实测（山东|N550）：
         · 2026-09-03  low=9000  high=9300  → 自算中值 9150，middlePrice=9200
         · 2026-09-17  low=11800 high=12300 → 自算中值 12050，middlePrice=12000
       **勿用自算中值代替**，否则引入新的口径偏差。
    ⚠️ 与日常取数的关系见 `lz_price_center.VARIETY_MAP` 的 `curve_daily`：
       queryPricePage 对区间型品种（丙烯/炭黑/防老剂/促进剂M/TMTD）不回「主流价」，
       若不标 `curve_daily`，日常会取到**区间下沿**，与本文回填的历史差约半个
       区间宽（炭黑实测 300 元/吨），拼接处造出假跳变。
