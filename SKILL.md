...
changelog:
  v3.6 (2026-09-18):
    - + 「📜 关键经验 → 🧰 工程协作坑」三条：① **同一文件禁止并行发多个编辑**（后写基于旧快照整文件回写，静默吃掉先前改动且都回报“成功”）② **参考材料只有截图时不要动手**（曾照图猜分析字段，拿到原文档发现整块全错）③ **后台任务「failed」先看是不是自己的收尾动作**（本地验证服务 `pkill` 会被记成 failed，判据 `lsof -ti :<端口>`，复核须 `curl --noproxy '*'` 防代理 502 假象）
    - 这三条原只散落在 changelog / 会话记录里，现提到正文常读区，避免下次重复踩
  v3.5 (2026-09-18):
    - 🔴 修**隆众同源双接口口径分裂**（09-17 记为「未修复」）：`queryPricePage` 对**区间型品种**不回「主流价」键，`pick_price` 退到**区间下沿**；而历史回填走 `getSingleCurve` 取 `middlePrice`（主流价）→ 日常与历史差约半个区间宽（炭黑实测 200~300 元/吨），拼接处**造假跳变**。已在 `VARIETY_MAP` 给 5 项加 `"curve_daily": True`，日常也走曲线；新增 `fetch_variety_curve_daily()`，曲线失败**回退**日报 + WARNING
    - + 体检通道 `python3 -m jinggong_monitor.lz_price_center --verify`：逐品种比对日报口径与曲线口径，暴露未标记的漏网品种
    - ❗ `middlePrice` **不等于** `(low+high)/2` —— 是隆众独立维护的主流价（09-03 实测 mid=9200 vs 自算 9150；09-17 mid=12000 vs 自算 12050）。**不可自算代替**；`backfill_plastic_ext.py` 里「即区间中值」的错误注释已删
    - 全历史核对：隆众 15 项 × 约 13000 点逐日比对，**14 项 0 处不一致**；唯一例外 `BR9000` 09-17（看板 15400 vs 源 15200，源方发布后**回溯修订**，`dataRiseOrFall=0` 佐证）已按「源当前口径为准」修正。data.json 1552 → **1553 个交易日**
    - 更正 09-17 误判：当时列 7 项，实测 `ACC_DM`/`ACC_CZ` 两口径一致，属误报，未加标记
  v3.4 (2026-09-17 夜):
...
### 🧰 工程协作坑（2026-09-18 补，与抓取无关但同样坑人）

1. **禁止对同一文件并行发多个编辑** —— 后续编辑基于**旧快照**整文件回写，会静默吃掉先前的改动，**且两次都回报“成功”**（2026-09-18 实测被吃掉 5 处，页面白屏报 `exportStore is not defined`）。
   **正确做法**：一次只改一个文件；批量改动写成一个原子脚本（`str.replace` + `assert s.count(old) == 1` 断言命中次数），改完立刻 `node --check` / `ast.parse` 验语法。
2. **参考材料只有截图时不要动手** —— 曾照参考**截图**猜分析模块字段（猜成「当前值/上月同期/年同期 + 日周月季年涨跌幅」），拿到**原文档**一比**全错**。真实形态是「周均价 / 月均价 / 年度均价 + 较上周 / 较上月 / 较各历史年均价」，且**行=指标、列=品种**。
   教训：**要原文档，不要猜图**；猜出来的字段哪怕看着合理，也可能整块不在真实需求里。
3. **后台任务「failed」先看是不是自己的收尾动作** —— 本地 `python3 -m http.server` 起的验证服务，收尾 `pkill` 后平台会记成 `failed`，**不是故障**。判断：`lsof -ti :<端口>` 为空即已正常停止。
   ⚠️ 别被沙箱代理返回的 `502` 假象骗到 —— 复核要带 `curl --noproxy '*'`。
...
| **404 / 500（2026-09-18）** | **整站宕机**（`https://www.ccmn.cn/` 首页同为 500 openresty） | 先 `curl -o /dev/null -w "%{http_code}" https://www.ccmn.cn/` 判**整站 vs 单端点**；整站故障只能**留空**，由 17:00 补抓重试 |

> ⚠️ **ccmn 没有历史回补后路**：`getCorpStmarketPriceList` 的 `publishDate` **不生效**、恒返回最新一日；真实历史接口 `POST /metalquote/query` **需会员登录**。→ 当日取不到即**永久丢失**，不是"晚点补上"。
> ✅ 2026-09-18 起主流程各源已加**单源容错**（`daily_update_all.py` 的 `_safe_async/_safe_sync`）：单源故障不再中止整轮，失败源记入 `failed_sources` 并在结尾汇总；**全源皆空才放弃写表/发布**。
...
| **OCR 结果与历史差一个量级**（2026-09-18：890 读成 390） | **硬二值化把「8」系统性读成「3/9」**；日志的「N 票一致」只是同一误读跨窗口重复 | 已改**同窗 3 预处理变体多数表决**（见下）；**遇量级异常先读图再下结论**，然后用 `manual_fill_tungsten.py <价格>` 更正 |
| 抓不到 / 全乱码 | 当日图未发或站点限流 | 当日留空；`>20 次/分` 会 Connection refused，需降频 |
| 需要人工兜底 | — | `manual_fill_tungsten.py <价格>` |

**OCR 参数**（`_extract_w_from_image()`）：y 0.42~0.56 步进 0.01 × 窗高 0.055/0.06/0.065 × 微扰 ±2 → 放大 3 倍 → **3 变体预处理** → tesseract(chi_sim+eng)。
锚点 `2-10…[muμm]`，取价区间 200–5000；含「碳化」的窗口排除。

**🔴 预处理变体多数表决（2026-09-18 加固，commit `4a33941`）**
- 变体 = (放大 3×, 对比度 2.0) × { **不二值化** / 阈值 180 / 阈值 150 }，**同一窗口内**跑 3 次取多数。
- 规则：**≥2 变体同值 ⇒ 采用**；三变体各说各话 ⇒ **弃权留空**（宁留空不取错，守铁律）；仅 1 变体命中 ⇒ 采用。
- 为何改：原「≥2 票一致」只在**同一变体跨窗口**成立——同一字形被系统性误读时，重复扫描只会把**同一个错误**投成多票，**不构成正确性证据**。09-18 实测：阈值 180 → `900.00`（错）；不二值化 / 阈值 150 → `890.00`（对）。
- 另：扫描循环改为「窗高·微扰在外层、纵向位置在内层」，让首轮即扫遍全表 y 区间（OCR 预算 `MAX_CALLS=45` 次）。回归实测 11s 得 890.00。

> **通用判据**：OCR 的「多窗投票一致」只防**随机**误差，不防**字形系统性误读**；真正的交叉验证必须跨**不同预处理链路**。凡 OCR 值触发上述量级/形近（8↔3、8↔9）可疑，**先下载图片目视核对**，再动数据。
...
| `jinggong_monitor/lz_price_center.py` | 隆众 `dc.oilchem.net` 结构化价格库封装（cookie 直连），详见 E.2。体检入口：`--check`（登录态 + 试抓全品种）/ `--verify`（**比对日报口径 vs 曲线口径**，暴露口径分裂） |
...
- **data.json 现共 34 品种 = 塑料 13 + 扩品类 20 + 原油 1**（2026-09-17 扩充并上线），**1553 个交易日（2020-07-01 ~ 2026-09-18）**。塑料 13（PC×3、ABS×3、PP×5、POE×1、PA6×1）口径＝余姚中国塑料城市场参考价（元/吨），受源站**滚动一年窗口**限制只能回溯到 2025-09-17；扩品类 20 的源与口径见 E.2。
